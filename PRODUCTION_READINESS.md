...
The supported surface is **Robotic choreography observation** at `/api/governed-robotic-runs`. It provides tenant- and subject-scoped cases, opaque evidence digests, deterministic assessments, optimistic state transitions, dual control, immutable event history, and connector-failure receipts.
...
The policy endpoint reports declared connectors as unconfigured. No repository, CI, model, telemetry, secret manager, artifact, ticketing, device gateway, robot, or other hardware integration is configured or validated. Live device WebSocket control remains disabled.
...
2. Review `backend/migrations/001_governed_robotic_run.sql` and apply it manually in a controlled change window:
   `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/migrations/001_governed_robotic_run.sql`
...
4. Install locked dependencies explicitly in `backend` and `frontend`; review vulnerabilities and licenses.
5. Run `node --test backend/governance/*.test.cjs`, syntax checks, and `bash -n start.sh`.
