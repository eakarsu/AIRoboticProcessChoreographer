// AI Dynamic fleet size optimization
// AI predicts workload, recommends ideal robot count
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIRoboticProcessChoreographer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIRoboticProcessChoreographer. Focus area: Dynamic fleet size optimization. ${`AI predicts workload, recommends ideal robot count`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Dynamic fleet size optimization.\n${`AI predicts workload, recommends ideal robot count`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'fleet-sizing', context, parsed);
    res.json({ feature: 'fleet-sizing', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[fleet-sizing]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['fleet-sizing']);
